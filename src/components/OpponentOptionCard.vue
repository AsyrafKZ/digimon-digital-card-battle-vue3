...
import { PHASE, CARD_NAME, WHO, MODE } from "../const/const.js";
import { hexToRgb } from "../utils/mapper"
import { addPulseEffect, removePulseEffect, setCardToInfoBoard, setOppSupport } from "./fieldManager";
...
const onPointerOver = (e) => {
  ...
  if (oppStore.oActiveMonStackCount > 0) {
    const activeMonStackTop = oppStore.oActiveMonStack[oppStore.oActiveMonStackCount - 1].name
    if (oppStore.oActiveMonStack.map(el => el.name).indexOf(props.card.name) > -1
